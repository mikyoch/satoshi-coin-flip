...
module games_with_chance::satoshi_flip {
    ...
    */
    ...
    const ECoinBalanceNotEnough: u64 = 9; // House provided coin should match the max_bet exactly.
    ...
    public entry fun start_game(hash: vector<u8>, coin: Coin<SUI>, min_bet: u64, max_bet: u64, ctx: &mut TxContext) {
        assert!(min_bet > 0, EZeroMinBet);
        assert!(max_bet >= min_bet, EMinBetTooHigh);
        assert!(coin::value(&coin) >= max_bet, ECoinBalanceNotEnough);
        let house_coin = give_change(coin, max_bet, ctx);
    ...
    public entry fun play(game: &mut Game, guess: u8, coin: Coin<SUI>, stake_ammount: u64, ctx: &mut TxContext) {
        ...
        let house_data = option::borrow(&game.house_data);
        assert!(stake_ammount <= house_data.max_bet, EStakeTooHigh);
        assert!(stake_ammount >= house_data.min_bet, EStakeTooLow);
